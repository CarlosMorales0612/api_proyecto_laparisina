...
async function crearEmpleado(req, res) {
  ...
  const cuentaExpReg = /^[0-9]{7,15}$/;
  const longitudMaximaCuenta = 15;
  ...
  if (!letrasExpReg.test(nombre_empleado)) {
    return res
      .status(400)
      .json({ error: "El campo nombre empleado solo   permite letras." });
  }
  if (nombre_empleado.length > longitudMaximaLetras) {
    return res
      .status(400)
      .json({
        error: "El campo nombre empleado debe tener máximo 20 caracteres.",
      });
  }
  if (!letrasExpReg.test(tipo_contrato_empleado)) {
    return res
      .status(400)
      .json({ error: "El campo tipo contrato empleado solo permite letras." });
  }
  if (tipo_contrato_empleado.length > longitudMaximaLetras) {
    return res
      ...
      .json({
        error:
          "El campo tipo contrato empleado debe tener máximo 20 caracteres.",
      });
  }
  if (!letrasExpReg.test(tipo_documento_empleado)) {
    return res
      .status(400)
      .json({ error: "El campo tipo documento empleado solo permite letras." });
  }
  if (tipo_documento_empleado.length > longitudMaximaLetras) {
    return res
      ...
      .json({
        error:
          "El campo tipo documento empleado debe tener máximo 20 caracteres.",
      });
  }
  if (!letrasExpReg.test(lugar_nacimiento_empleado)) {
    return res
      .status(400)
      .json({
        error: "El campo lugar nacimiento empleado solo permite letras.",
      });
  }
  if (lugar_nacimiento_empleado.length > longitudMaximaLetras) {
    return res
      ...
      .json({
        error:
          "El campo lugar nacimiento empleado debe tener máximo 20 caracteres.",
      });
  }
  if (!letrasExpReg.test(municipio_domicilio_empleado)) {
    return res
      .status(400)
      .json({
        error: "El campo municipio domicilio empleado solo   permite letras.",
      });
  }
  if (municipio_domicilio_empleado.length > longitudMaximaLetras) {
    return res
      ...
      .json({
        error:
          "El campo municipio domicilio empleado debe tener máximo 20 caracteres.",
      });
  }
  if (!letrasExpReg.test(estado_civil_empleado)) {
    return res
      .status(400)
      .json({ error: "El campo estado civil empleado solo   permite letras." });
  }
  if (estado_civil_empleado.length > longitudMaximaLetras) {
    return res
      ...
      .json({
        error:
          "El campo estado civil empleado debe tener máximo 20 caracteres.",
      });
  }
  if (!letrasExpReg.test(alergia_empleado)) {
    return res
      .status(400)
      .json({ error: "El campo alergia empleado solo permite letras." });
  }
  if (alergia_empleado.length > longitudMaximaLetras) {
    return res
      .status(400)
      .json({
        error: "El campo alergia empleado debe tener máximo 20 caracteres.",
      });
  }
  if (!letrasExpReg.test(eps_empleado)) {
    return res
      .status(400)
      .json({ error: "El campo eps empleado solo   permite letras." });
  }
  if (eps_empleado.length > longitudMaximaLetras) {
    return res
      .status(400)
      .json({
        error: "El campo campo eps empleado debe tener máximo 20 caracteres.",
      });
  }
  if (!letrasExpReg.test(pension_empleado)) {
    return res
      .status(400)
      .json({ error: "El campo pension empleado solo   permite letras." });
  }
  if (pension_empleado.length > longitudMaximaLetras) {
    return res
      .status(400)
      .json({
        error: "El campo pension empleado debe tener máximo 20 caracteres.",
      });
  }
  ...
  if (!cuentaExpReg.test(cuenta_bancaria_empleado)) {
    return res
      .status(400)
      .json({ error: "El campo area empleado solo   permite letras." });
  }
  if (cuenta_bancaria_empleado.length > longitudMaximaCuenta) {
    return res
      .status(400)
      .json({
        error: "El campo area empleado debe tener máximo 20 caracteres.",
      });
  }
  if (!codigoExpReg.test(codigo_rotulacion_empleado)) {
    return res
      .status(400)
      .json({ error: "El campo código del empleado solo   permite numeros." });
  }
  if (codigo_rotulacion_empleado.length > longitudMaximaCodigo) {
    return res
      .status(400)
      .json({
        error: "El campo código del empleado debe tener máximo 4 caracteres.",
      });
  }
  if (!numerosExpReg.test(identificacion_empleado)) {
    return res
      ...
      .json({
        error:
          "El campo cedula del empleado solo   permite numeros, sin ningun signo.",
      });
  }
  if (identificacion_empleado.length > longitudMaximaNumeros) {
    return res
      .status(400)
      .json({
        error: "El campo cedula del empleado debe tener máximo 10 caracteres.",
      });
  }
  ...
  if (!edadExpReg.test(edad_empleado)) {
    return res
      .status(400)
      .json({ error: "El campo edad del empleado solo   permite numeros" });
  }
  if (edad_empleado.length > longitudMaximaEdad) {
    return res
      .status(400)
      .json({
        error: "El campo código del empleado debe tener máximo 3 caracteres.",
      });
  }
  ...
  if (!numerosExpReg.test(celular_empleado)) {
    return res
      ...
      .json({
        error:
          "El campo celular del empleado solo   permite numeros, sin ningun signo.",
      });
  }
  if (celular_empleado.length > longitudMaximaNumeros) {
    return res
      .status(400)
      .json({
        error: "El campo celular del empleado debe tener máximo 10 caracteres.",
      });
  }
  ...
  if (!grupoSanguineoExpReg.test(grupo_sanguineo_emeplado)) {
    return res
      ...
      .json({
        error:
          "Grupo sanguíneo no válido. Debe ser uno de los siguientes: A+, A-, B+, B-, AB+, AB-, O+, O-",
      });
  }
...
async function actualizarEmpleado(req, res) {
  ...
  try {
    ...
    // Verifica si empleado fue encontrado y actualizada correctamente
    if (!empleadoActualizado) {
      return res.status(404).json({ error: "Empleado no encontrado." });
    }

    // Si el empleado se actualiza exitosamente, envía un mensaje de éxito en la respuesta
    res
      .status(200)
      .json({
        message: "Empleado actualizado exitosamente.",
        empleados: empleadoActualizado,
      });
  } catch (error) {
    res.status(500).json({ error: "Error al actualizar la empleado." });
  }
}
